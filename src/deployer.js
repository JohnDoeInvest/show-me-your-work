  ...
  if (checkSuite.status === 'completed' && checkSuite.conclusion === 'success') {
    if (config.deployPullRequest === true && checkSuite.pull_requests.length > 0) {
      ...
      if (!config.staticBranches.includes(checkSuite.head_branch)) {
        addToQueue(COMMAND_REMOVE, utils.getIdFromBranch(config, checkSuite.head_branch), config)
      }

      addToQueue(COMMAND_DEPLOY, deployId, config, { cloneUrl: payload.repository.clone_url, branch: checkSuite.head_branch, sha: checkSuite.head_sha })
